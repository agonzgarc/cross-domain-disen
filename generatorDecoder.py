    ...
    if a.red_images:
        if a.mode == "train":
            layer_specs = [
                ...
                (a.ngf * 2, 0.5),   # decoder_6: [batch, 4, 4, ngf * 4 * 2] => [batch, 8, 8, ngf * 2 * 2]
                (a.ngf, 0.0),   # decoder_5: [batch, 8, 8, ngf * 2 * 2] => [batch, 16, 16, ngf  * 2]
            ]
        else:
            layer_specs = [
                (a.ngf * 4, 0.0),   # decoder_5: [batch, 1, 1, ngf * 4] => [batch, 2, 2, ngf * 4 * 2]
                (a.ngf * 4, 0.0),   # decoder_7: [batch, 2, 2, ngf * 4 * 2] => [batch, 4, 4, ngf * 4 * 2]
                # We could remove the dropout from the following one, maybe
                (a.ngf * 2, 0.0),   # decoder_6: [batch, 4, 4, ngf * 4 * 2] => [batch, 8, 8, ngf * 2 * 2]
                (a.ngf, 0.0),   # decoder_5: [batch, 8, 8, ngf * 2 * 2] => [batch, 16, 16, ngf  * 2]
            ]
        ...
        for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
            ...
            with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
                if decoder_layer == 0:
                    ...
                    rectified = tf.concat([sR,eR],axis=3)
                else:
        ...
        if a.mode == "train":
            layer_specs = [
                ...
                (a.ngf * 4, 0.5),   # decoder_5: [batch, 8, 8, ngf * 8 * 2] => [batch, 16, 16, ngf * 8 * 2]
                (a.ngf * 2, 0.5),   # decoder_4: [batch, 16, 16, ngf * 8 * 2] => [batch, 32, 32, ngf * 4 * 2]
                (a.ngf, 0.0),   # decoder_3: [batch, 32, 32, ngf * 4 * 2] => [batch, 64, 64, ngf * 2 * 2]
        ...
        # Tile the exclusive vector to match size of feature map
        eRT = tf.tile(eR, [1, 8, 8, 1])

        #with tf.variable_scope("decoder_resnet"):
            #inputRes = n_res_blocks(tf.concat([sR,eRT],axis=3),3)
